...
- [ ] 8. Manual acceptance verification against the Phase 1 accept criteria: open a file, scroll with the full key row, quit with `q` and with `Ctrl-C`, confirm an injected `panic!()` still leaves the terminal sane, confirm piping stdout errors out per Section 3. Record the outcome in the PR description.
